...
class GuitarScene(BandPlayBaseScene):
                ...
                if instrument.freestyleActive or instrument.drumFillsActive:
                    ...
                    if missedNotes:
                        ...
                        for tym, theNote in missedNotes:
                            log.debug("RTMA Message - Missed Note: note: %s target time: %s" % (theNote.number, tym))
                            out_msg = RTMA.CMessage(RTMA_types.MT_FOFIX_MISSED)
                            msg_data = RTMA_types.MDF_FOFIX_MISSED()
                            msg_data.note = theNote.number
                            msg_data.target_time = tym
                            msg_data.game_time = self.songTime
    ...
    def doPick3GH2(self, num, hopo=False, pullOff=False):  # so DoPick knows when a pull-off was performed
        ...
        # active key to note mapping:
        activeNoteList = []
        for k in activeKeyList:            
            activeNoteList.append((k.bit_length() - 7) / 2)
        
        
        self.killswitchEngaged[num] = False  # always reset killswitch status when picking / tapping
        if self.instruments[num].startPick3(self.song, self.songTime, self.controls, hopo):
            log.debug("RTMA Message - User Input: notes strummed: %s game time: %s hit note: True" % (activeNoteList, self.songTime))
            out_msg = RTMA.CMessage(RTMA_types.MT_FOFIX_INPUT)
            msg_data = RTMA_types.MDF_FOFIX_INPUT()
            for note in activeNoteList:
	            msg_data.notes_strummed[note] = 1
            msg_data.game_time = self.songTime
            msg_data.hit_note = True
            RTMA.copy_to_msg(msg_data, out_msg)
            self.engine.rtmaModule.SendMessage(out_msg)
            self.noteHit(num)
        else:
            log.debug("RTMA Message - User Input: notes strummed: %s game time: %s hit note: False" % (activeNoteList, self.songTime))  
            out_msg = RTMA.CMessage(RTMA_types.MT_FOFIX_INPUT)
            msg_data = RTMA_types.MDF_FOFIX_INPUT()
            for note in activeNoteList:
	            msg_data.notes_strummed[note] = 1
            msg_data.game_time = self.songTime
            msg_data.hit_note = False
            RTMA.copy_to_msg(msg_data, out_msg)
            self.engine.rtmaModule.SendMessage(out_msg)  
            ApplyPenalty = True
