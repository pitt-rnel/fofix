...
class Guitar(Instrument):
    ...
    def run(self, ticks, pos, controls):
        ...
        self.upcomingNotes = self.getUpcomingNotes(self.scene.song, pos, ticks)
        import pdb
        for n in self.upcomingNotes:
            log.debug("RTMA Message - Prompt: Current Time: %d Notes: %d Target Time: %d" % (pos, n[1].number, n[0]))
